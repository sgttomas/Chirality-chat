...
import { parseCellValue, validateDS, validateSP, validateX, validateZ, validateM, type DS, type SP, type X, type Z, type M } from '@/lib/parseCellValue'
          ...
          <div key={index} className="border rounded-lg p-4 bg-white">
            <div className="flex items-start justify-between mb-3">
              <h5 className="font-semibold text-gray-800">
                Step {index + 1}: {parsed.step || cell.labels?.rowLabel}
              </h5>
              <Badge variant="outline">
                {parsed.duration || 'Variable'}
              </Badge>
            </div>
            
            <p className="text-gray-700 mb-3">{parsed.purpose || '—'}</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <h6 className="font-medium text-gray-600 mb-1">Inputs</h6>
                <ul className="list-disc list-inside space-y-1">
                  {parsed.inputs?.map((input: string, i: number) => (
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1">
                  {parsed.outputs?.map((output: string, i: number) => (
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1">
                  {parsed.preconditions?.map((pre: string, i: number) => (
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1">
                  {parsed.postconditions?.map((post: string, i: number) => (
            ...
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center space-x-2">
                <input 
                  ...
                  defaultChecked={parsed.status === 'completed'}
                />
                <h5 className="font-medium text-gray-800">
                  {parsed.item || cell.labels?.rowLabel}
                </h5>
              </div>
              <Badge variant={getSeverityColor(parsed.severity)}>
                {parsed.severity || 'Unknown'}
              </Badge>
            </div>
            
            <p className="text-gray-700 text-sm mb-2">{parsed.rationale || '—'}</p>
            
            <div className="space-y-2 text-sm">
              <div>
                <span className="font-medium text-gray-600">Acceptance Criteria: </span>
                <span className="text-gray-700">{parsed.acceptanceCriteria || '—'}</span>
              </div>
              
              <div>
                <span className="font-medium text-gray-600">Evidence: </span>
                <span className="text-gray-700">{parsed.evidence || '—'}</span>
              </div>
            </div>
          </div>
        )
        ...
        return (
          <div key={index} className="border rounded-lg p-4 bg-white">
            <h5 className="font-semibold text-gray-800 mb-3">
              Solution {index + 1}: {parsed.statement || cell.labels?.rowLabel}
            </h5>
            
            <div className="space-y-3 text-sm">
              <div>
                <h6 className="font-medium text-gray-600 mb-1">Justification</h6>
                <p className="text-gray-700">{parsed.justification || '—'}</p>
              </div>
              
              <div>
                <h6 className="font-medium text-gray-600 mb-1">Trace Back</h6>
                <ul className="list-disc list-inside space-y-1">
                  {parsed.traceBack?.map((trace: string, i: number) => (
              ...
              <div>
                ...
                <p className="text-gray-700">{parsed.residualRisk || '—'}</p>
              </div>
            </div>
          </div>
        )
        ...
        return (
          <div key={index} className="mb-6 pb-4 border-b border-gray-200 last:border-b-0">
            <h5 className="text-lg font-semibold text-gray-800 mb-3">
              {parsed.heading || cell.labels?.rowLabel}
            </h5>
            
            <div className="text-gray-700 mb-4 whitespace-pre-line">
              {parsed.narrative || 'No content available'}
            </div>
            
            {(parsed.precedents?.length || parsed.successors?.length) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  <h6 className="font-medium text-gray-600 mb-2">Precedents</h6>
                  <ul className="list-disc list-inside space-y-1">
                    {parsed.precedents?.map((prec: string, i: number) => (
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {parsed.successors?.map((succ: string, i: number) => (
          ...
          {data.map((cell, index) => {
            ...
            return (
              <tr key={index} className="hover:bg-gray-50">
                ...
                <td className="border border-gray-200 px-4 py-2">
                  <Badge variant="outline">
                    {parsed.fromVersion || 'V?'} → {parsed.toVersion || 'V?'}
                  </Badge>
                </td>
                <td className="border border-gray-200 px-4 py-2">
                  {parsed.change || '—'}
        ...
        return (
          <div key={index} className="border rounded-lg p-4 bg-white">
            <div className="flex items-center justify-between mb-3">
              <h5 className="font-semibold text-gray-800">
                Round {parsed.round || index + 1} Synthesis
              </h5>
              <div className="flex items-center space-x-2">
                <Badge variant={parsed.convergence >= 0.8 ? 'default' : 'destructive'}>
                  Convergence: {(parsed.convergence * 100 || 0).toFixed(1)}%
                </Badge>
                <Badge variant="outline">
                  Confidence: {(parsed.confidence * 100 || 0).toFixed(1)}%
                </Badge>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <ul className="list-disc list-inside space-y-1">
                  {parsed.openIssues?.map((issue: string, i: number) => (
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1">
                  {parsed.nextActions?.map((action: string, i: number) => (
          ...
          <div key={index} className="border rounded-lg p-4 bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1">
                  {parsed.features?.map((feature: string, i: number) => (
